...
ValueType norm2(const matrix_info info, ValueType *data)
{
    return std::sqrt(reduce<ValueType>(
        info, data,
        [](ValueType o1, ValueType o2) { return o1 * o1 + o2 * o2; }));
}


template <typename ArType>
void apply_pivot(ArType *data, const matrix_info &m_info, int *pivot)
{
    for (std::size_t col = 0; col < m_info.size[1]; ++col) {
        const std::size_t new_col = pivot[col];
        for (std::size_t row = 0; row < m_info.size[0]; ++row) {
            const std::size_t idx = row + m_info.stride + col;
            const std::size_t new_idx = row + m_info.stride + new_col;
...
int main(int argc, char **argv)
{
    ...
    constexpr bool measure_error{false};
    ...
    constexpr std::size_t max_rows{4 * 1000};
    ...
    auto cpu_rhs = gen_mtx<ar_type>({{max_cols, 1}, 1}, vector_dist, rengine)
        Memory<ar_type>
            gpu_rhs(Memory<ar_type>::Device::gpu, max_cols);
    gpu_rhs.copy_from(cpu_rhs);
    ...
    auto ar_data =
        IrMemory<ar_type>(max_rows, max_cols, cpu_mtx_orig, cpu_vect_gen);
    auto st_data = IrMemory<st_type>(ar_data);

    // permutate the original MTX, so it matches the LU decomposition
    apply_pivot(cpu_mtx_orig.data(), max_size, ar_data.cpu_pivot());
        ...
        benchmark_info_t{"TRSV fp64",
                         [&](matrix_info m_info, matrix_info x_info) {
                             ...
                             trsv_3(m_info, t_matrix_type2, d_matrix_type2,
                                    ar_data.gpu_mtx_const(), x_info,
                                    ar_data.gpu_x(), trsv_helper.data());
                             update_vector(ar_data.gpu_x(), x_info,
    ...
    for (int b_idx = 0; b_idx < benchmark_num; ++b_idx) {
        for (int i = 0; i < max_iter; ++i) {
            ...
            gpu_res.copy_from(gpu_rhs);
            gemv(m_info, -1, gpu_mtx_orig.const_data(), x_info,
                 gpu_dp_x.const_data(), x_info, 1, gpu_dp_res.data());
            cpu_res.copy_from(gpu_res);
            auto norm = norm2(x_info, cpu_res.data());
